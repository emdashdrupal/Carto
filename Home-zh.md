...
| :------ | ------ | ----| ----------- | -----|
| GeoJSON | `.json` | 向量 | 區域、建築、網路、分區 | 結構簡單、輕量化，可以使用一般的文本編輯器編輯。適合少量資料的儲存與交換，但是缺乏對各圖徵（Feature）的編碼，導致搜尋與渲染效率不高。 |
| Shapefile | `.shp`、`.shx`、`.dbf` 等 | 向量 | 區域、建築、網路、分區 | 二進位檔案不容易編輯，且需要數個附隨檔案（[Sidecar file](https://en.wikipedia.org/wiki/Sidecar_file)） 配合才能讀取。由於已對各圖徵編碼，搜尋與渲染效率較高。|
| GeoTIFF | `.tif` | 網格 | 地形、水體 | 儲存連續資料的影像檔案，可以用主流的媒體檢視器（如 Microsoft 相片）開啟。|

簡單來說，如果希望輸出**建築物**、**道路**、**軌道**、**小徑**、**行政區邊界**、**地圖區塊邊界**或**分區單元**這類項目，你應該選擇向量的 GeoJSON 或 Shapefile；如果希望輸出**水域範圍**或**地形高度**，那麼你應該選擇網格的 GeoTIFF。
...
| --------- | ---------| ---------------|--------------------------|------|
| 家庭／居民 | ✅       | ⚠️僅居於公園者 | ❌                       |由於無法找到公園居住的無家可歸者居無定所，因此可被排除。|
